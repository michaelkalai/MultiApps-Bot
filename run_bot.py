  ...
  async def connect4(ctx, *args):
      ...
      while con == True:
          ...
          if player == connect.user1:
              msg = await bot.wait_for("message", check=check_1, timeout = 86400)
          elif player == connect.user2:
              msg = await bot.wait_for("message", check=check_2, timeout = 86400)
          else:
              continue
          col = msg.content
          try:
...
# Jordan's protein intake calculator
  ...
  async def protein(ctx):
    ...
    if float(body_weight) < 0:
      await ctx.send("Negatives numbers aren't applicable, please type in your body weight:")

    if float(body_weight) > 0:
      bodyweight = float(body_weight) * 0.7
      await ctx.send(f"Your daily intake should be {round(bodyweight)} grams of protein for muscle building")
  ...
  async def hangman(ctx, args):
    ...
    def check_player(msg):
      return str(msg.author) == hangman.player and msg.channel == ctx.channel and len(msg.content) < 2 and msg.content.isalpha() and msg.content.lower() not in hangman.used_letters
    ...
    while True:
      ...
      if len(msg.content) > 40:
        await ctx.author.send("Words/phrases are limited to 40 characters")
      for chr in word:
    ...
    await ctx.send(f"<@{ctx.author.id}> has challenged <@{playerid}> to hangman")
    ...
    while True:
      ...
      msg = await bot.wait_for("message", check = check_player, timeout = 86400)
      guess = msg.content.lower()
      if not hangman.check_letter(guess):
        hangman.lives -= 1
      elif hangman.check_completion():
        await ctx.send(embed=hangman.embed())
        await ctx.send(f"Congratulations <@{playerid}>, You completed the word!")
        break
      if hangman.lives < 1:
        await ctx.send(f"Out of lives! The correct word/phrase was \"{word}\"")
        break
